...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, _rawListParams, _signal, _onUpdate, ctx): Promise<any> {
      ...
      if (agents.length === 0) {
        return { content: [{ type: "text" as const, text: "No agents found in the configured scope." }] };
      }

      const list = agents.map((a: AgentConfig) => {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, _rawParams, signal, onUpdate, ctx): Promise<any> {
      ...
      const isSubagentEnv = !!(process.env.SUBAGENT_IS_RUNNING || process.env.SUBAGENT_NAME);
      ...
      if (isSubagentEnv || isSubagentFile) {
        ...
        return {
          content: [{ type: "text", text: `❌ BLOCKED: subagent cannot be called from within a subagent. This is working as intended — NOT a configuration error. You are the ${agentName} agent. Do NOT retry subagent — every attempt will be blocked. Do NOT read SKILL.md or other agents' files. Complete your specific job now using read, write, bash, grep, and find.` }],
        };
      }
      ...
      if (typeof params === "string") {
        ...
        params = { mode: "single", agent: parts[0], task: parts.slice(1).join(" ") };
        ...
        // Two-key object — likely positional args passed as {0: agent, 1: task} or similar
          ...
          params = { mode: "single", agent: params[keys[0]] || "", task: params[keys[1]] || "" };
        }
        ...
        // Single-key object — LLM passed positional args as {"agentName": "task"}
          ...
          params = { mode: "single", agent: keys[0] || "", task: String(params[keys[0]] || "") };
        }
      }
      ...
      if (!params?.mode) {
        return {
          content: [{ type: "text", text: "Missing 'mode' parameter. Use: single, chain, parallel, or batch." }],
        };
      }
      ...
      const settings = loadSubagentsSettings(discovery.projectAgentsDir);

      const mode = params.mode;
      const hasChain = mode === "chain" && (params.chain?.length ?? 0) > 0;
      const hasTasks = mode === "parallel" && (params.tasks?.length ?? 0) > 0;
      const hasSingle = mode === "single" && Boolean(params.agent && params.task);
      const hasBatch = mode === "batch" && (params.stages?.length ?? 0) > 0;
      const modeCount = Number(hasChain) + Number(hasTasks) + Number(hasSingle) + Number(hasBatch);
      ...
      const concept = params.task || params.chain?.[0]?.task?.slice(0, 100) || "Pipeline";

      const makeDetails = (mode: "single" | "chain" | "parallel" | "batch") =>
      ...
      if (modeCount !== 1) {
        const available = agents.map((a: AgentConfig) => `"${a.name}"`).join(", ");
        return {
          content: [{ type: "text", text: `Provide exactly one mode. Available agents: ${available || "none"}` }],
      ...
      if (params.stages && params.stages.length > 0) {
        const batchResults: { stage: string; results: SingleResult[]; status: "done" | "stopped"; successCount: number; failedCount: number }[] = [];

        initEvents(eventsFile, concept, "batch-pipeline");

        for (let stageIdx = 0; stageIdx < params.stages.length; stageIdx++) {
          const stage = params.stages[stageIdx];
          const stageCwd = params.cwd || extractProjectCwd(stage.tasks[0]?.task || "") || subagentCwd;
          const stageTimeout = stage.timeout ?? globalTimeout;
          const stageOutputFormat = stage.outputFormat ?? globalOutputFormat;
          ...
          const stageResults: SingleResult[] = new Array(stage.tasks.length);
          for (let i = 0; i < stage.tasks.length; i++) {
            stageResults[i] = {
              agent: stage.tasks[i].agent, agentSource: "unknown", task: stage.tasks[i].task,
          ...
          const emitBatchUpdate = () => {
            if (onUpdate) {
              const done = stageResults.filter((r) => r.exitCode !== -1).length;
              const running = stageResults.filter((r) => r.exitCode === -1).length;
              onUpdate({
                content: [{ type: "text", text: `Stage "${stage.name}": ${done}/${stage.tasks.length} done, ${running} running...` }],
                details: makeDetails("batch")([...batchResults.map((b) => b.results).flat(), ...stageResults.filter((r) => r.exitCode !== -1)]),
              });
            }
          };

          await mapWithConcurrencyLimit(stage.tasks, getMaxConcurrency(settings), async (t: any, index) => {
            if (signal?.aborted) return null as any;
            const result = await runSingleAgent(
              agents, t.agent, t.task, stageCwd, eventsFile, stage.name, undefined, signal,
              (partial) => {
                if (partial.details?.results[0]) {
                  stageResults[index] = partial.details.results[0];
                  emitBatchUpdate();
                }
              },
              makeDetails("batch"),
          ...
          const successCount = stageResults.filter((r) => r.exitCode === 0).length;
          ...
          batchResults.push({
            ...
            status: failed.length > 0 ? "stopped" as const : "done" as const,
            successCount,
            failedCount: failed.length,
          });

          if (failed.length > 0) {
            const failNames = failed.map((f) => `${f.agent} (${f.stopReason || f.exitCode})`).join(", ");
            appendEvent(eventsFile, {
              time: new Date().toISOString(),
              agent: "system",
              phase: stage.name,
            ...
            return {
              content: [{ type: "text", text: `Batch stopped at stage "${stage.name}": ${failNames}` }],
              details: makeDetails("batch")([...batchResults.map((b) => b.results).flat()]),
              isError: true,
            };
          }
        }
        ...
        const summary = batchResults.map((b) => `Stage "${b.stage}": ${b.successCount}/${b.results.length} tasks completed`).join("\n");

        return {
          content: [{ type: "text", text: `Batch pipeline complete:\n\n${summary}\n\n${allResults.length} total tasks across ${batchResults.length} stages` }],
        ...
        for (let i = 0; i < params.chain.length; i++) {
          ...
          if (i > 0 && results.length > 0) {
            ...
            const preview = prevOutput
              ...
              .replace(/\s+/g, " ")
              .slice(0, 200);
            const status = prevResult.exitCode === 0 ? "completed" : `failed (${prevResult.stopReason || prevResult.exitCode})`;
            const replacement = `[${status}] ${preview}`;
            compressedTask = step.task.replace(/\{previous\}/g, replacement);
          }

          const stepTimeout = step.timeout ?? globalTimeout;
          const stepOutputFormat = step.outputFormat ?? globalOutputFormat;
          ...
          const chainUpdate: OnUpdateCallback | undefined = onUpdate
            ? (partial) => {
                ...
                if (currentResult) {
                  onUpdate({ content: partial.content as any, details: makeDetails("chain")([...results, currentResult] as any) as any });
                }
              }
            : undefined;

          const result = await runSingleAgent(
            agents, step.agent, compressedTask, stepCwd, eventsFile, `step-${i + 1}`, i + 1, signal, chainUpdate, makeDetails("chain"),
            settings, stepTimeout, stepOutputFormat, step.thinkingLevel ?? params.thinkingLevel,
          );
          results.push(result);
          ...
          if (isResultError(result)) {
            ...
            return {
              content: [{ type: "text", text: `Chain failed at step ${i + 1} (${step.agent}): ${errorMsg}` }],
      ...
      if (params.tasks && params.tasks.length > 0) {
        if (params.tasks.length > getMaxParallel(settings)) {
          return {
            content: [{ type: "text", text: `Max ${getMaxParallel(settings)} parallel tasks. Got ${params.tasks.length}.` }],
        ...
        const results = await mapWithConcurrencyLimit(params.tasks, getMaxConcurrency(settings), async (t: any, index) => {
          if (signal?.aborted) return null as any;
          const stepTimeout = t.timeout ?? globalTimeout;
          const stepOutputFormat = t.outputFormat ?? globalOutputFormat;
          const stepThinkingLevel = t.thinkingLevel ?? params.thinkingLevel;
          const result = await runSingleAgent(
            agents, t.agent, t.task, subagentCwd, eventsFile, "parallel", undefined, signal,
            (partial) => {
              if (partial.details?.results[0]) {
      ...
      if (params.agent && params.task) {
        ...
        const result = await runSingleAgent(
          agents, params.agent, params.task, subagentCwd, eventsFile, params.agent, undefined, signal, onUpdate as any, makeDetails("single"),
      ...
      return {
        content: [{ type: "text", text: `Invalid parameters. Available agents: ${agents.map((a: AgentConfig) => `"${a.name}"`).join(", ") || "none"}` }],
        details: makeDetails("single")([]),
      };
    },

    renderCall: (args, theme, _context) => renderCall(args, theme),
    renderResult: (result, { expanded }, theme, _context) => renderResult(result as any, { expanded }, theme),
  });
}
